...
class Faqs_RUD(Resource):
    ...
    def get(self, faq_id):
        ...
        if faq_object:
            ret = Faq_Schema().dump(faq_object)
    ...
    def delete(self, faq_id):
        ...
        if user_status in ["director", "organizer"]:
            try:
                faq_to_delete = g.session.query(
                    g.Base.classes.faqs).get(faq_id)
                if faq_to_delete:
                    # this makes sure that at least one faq matches faq_id
                    g.session.query(g.Base.classes.faqs).filter(
                        g.Base.classes.faqs.id == faq_id).delete()
                    return ("", 204, headers)
                else:
                    return (not_found, 404, headers)
            except Exception as err:
...
class Faqs_CR(Resource):
    ...
    def post(self):
        ...
        try:
            exist_check = g.session.query(exists().where(and_(
                g.Base.classes.faqs.question == data["question"], g.Base.classes.faqs.answer == data["answer"]))).scalar()
            if exist_check:
        ...
        if user_status in ["director", "organizer"]:
            try:
                ...
                new_faq = g.session.query(g.Base.classes.faqs).filter(
                    g.Base.classes.faqs.question == data["question"]).one()
    ...
    def get(self):
        ...
        try:
            ...
            ret = []
            for faq in all_faqs:
                ret.append(Faq_Schema().dump(faq))
            return (ret, 200, headers)
        except Exception as err:
            print(type(err))
            print(err)
