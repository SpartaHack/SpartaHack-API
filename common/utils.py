...
unprocessable_entity = {"status": 422,
                        "message": "Unprocessable Entity", "error_list": {}}
internal_server_error = {"status": 500,
                         "message": "Internal Server Error", "error_list": {}}
...
def has_admin_privileges():
    ...
    role = ["director", "judge", "mentor", "sponsor",
            "organizer", "volunteer", "hacker"]
    if user:
...
def send_email(subject, recipient, body):
    try:
        ...
        mail_server = smtplib.SMTP(
            host=app.config["MAIL_SERVER"], port=app.config["MAIL_PORT"])
        mail_server.starttls()
        mail_server.login(app.config["MAIL_USERNAME"],
...
def validate_ID_Token(id_token):
    ...
    jsonurl = urlopen("""https://"""+os.getenv("AUTH0_DOMAIN") +
